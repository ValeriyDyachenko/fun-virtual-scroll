...
class VScrollAPI {
	...
	private createFlyingField(field: HTMLElement, direction: 'up' | 'down'): void {
		...
		const randomY = direction === 'up' ? -window.innerHeight * 1.5 : window.innerHeight * 1.5;
		const randomRotate = (Math.random() - 0.5) * 720;

		requestAnimationFrame(() => {
			flyingField.style.transform = `translate(${randomX}px, ${randomY}px) rotate(${randomRotate}deg)`;
		});

		this.animationLayer.appendChild(flyingField);
		setTimeout(() => this.animationLayer.removeChild(flyingField), 1200);
	}
	...
	private animateFields(scrollDirection: 'up' | 'down', scrollSpeed: number): void {
		...
		animatedFields.forEach(field => this.createFlyingField(field, scrollDirection));
